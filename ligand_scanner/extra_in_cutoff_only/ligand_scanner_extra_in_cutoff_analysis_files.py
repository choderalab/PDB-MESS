...
def ligand_scanner(file):
    ...
    extra_in_cutoff_file_list = None
    ...
    for i in metal_atoms:
        ...
        if metal_ligand_dict_CORRECTED_in_cutoff_not_CONECT[i] and not metal_ligand_dict_in_CONECT_not_cutoff[i]:
            ...
            if not extra_in_cutoff_for_file_analyzed:
                ...
                extra_in_cutoff_file_list = file
...
def ligand_scanner_all_database(pdbpath):
    ...
    for dictionary_of_process_counts in pool.map(ligand_scanner, glob.iglob(pdbpath)):
        ...
        if dictionary_of_process_counts['extra_in_cutoff_file_list'] != None:
            dictionary_of_database_results['extra_in_cutoff_file_list'].append(dictionary_of_process_counts['extra_in_cutoff_file_list'])
...
# Write results
f = open('ligand_scanner_results_extra_in_cutoff_analysis_FILES.txt', 'w')
for line in dictionary_of_database_results['extra_in_cutoff_file_list']:
    f.write(str(line))
    f.write('\n')
