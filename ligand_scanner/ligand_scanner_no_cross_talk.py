...
def ligand_scanner(file):
    ...
    for i in metal_atoms:
        ligand_numbers.append((len(metal_ligand_dict_by_CONECT[i]), len(metal_ligand_dict_by_cutoff[i])))
        
        for j in metal_ligand_dict_by_CONECT[i]:
            CONECT_atoms.append((topo.atom(j).residue.name, topo.atom(j).name))
        
        for j in metal_ligand_dict_by_cutoff[i]:
            CUTOFF_atoms.append((topo.atom(j).residue.name, topo.atom(j).name))
