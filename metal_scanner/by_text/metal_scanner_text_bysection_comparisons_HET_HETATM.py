...
def metal_scanner(file):
    ...
    for line in pdbfile:
        ...
        if 'HETATM' in line and metal_name in line:
            metal_scanner_data[1] = 1    
        # early version where only excluded REMARK - see the complete list of lines_to_avoid_for_in_line_search    
...
## So: are there any [x,x,0] where x is 1    
    ...
    # Now looking only for HETATM not within HET, and HET not within HETATM, get files for which HETATM not within HET
    ...
    for metal_scanner_data in pool.map(metal_scanner, glob.iglob(pdbpath)):
        ...
        if metal_scanner_data[0] == 1 and metal_scanner_data[1] == 0:
            database_analyzer_output[0] += 1
            database_analyzer_output[2].append(metal_scanner_data[2]) 
        elif metal_scanner_data[0] == 0 and metal_scanner_data[1] == 1:            
            database_analyzer_output[1] += 1  
            database_analyzer_output[3].append(metal_scanner_data[2])          
...
with open('metal_scanner_results_by_text_bysection_comparison.txt', 'w') as f:
    ...
    f.write('HET not contained within HETATM:\n')
    f.write(str(database_analyzer_output[0]))
    f.write('\n')
    ...
    f.write('HETATM without HET files:\n')
    f.write('Number:\n')
