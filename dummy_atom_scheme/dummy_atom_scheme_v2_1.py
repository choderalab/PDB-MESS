...
def ligand_scanner(file):
    ...
    for i in metal_ligand_dict_by_cutoff:
        ...
        for j in metal_ligand_dict_by_cutoff[i]:
            ligand_analysis.append((topo.atom(j).residue, topo.atom(j).residue.chain.index))
        ...
        if len(ligand_analysis) == 5:
            ...
            for j in ligand_analysis:           
                five_coord_conect_per_atom[0].append((str(j[0]), j[1]))
        
            for k in metal_ligand_dict_by_CONECT[i]:
                five_coord_conect_per_atom[1].append(str(topo.atom(k)))      
                    
        
        five_coord_conect_per_atom[2] = five_coord_distances_per_atom
...
# Write results
f = open('./results/30Nov_dummy_atom_scheme_v2.1_results.txt', 'w')
