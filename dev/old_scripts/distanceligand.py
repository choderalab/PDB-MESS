...
# init
    try:
        ...
        with open("errors.txt", "rw+") as f:
            f.write("%s\n" % i)
        continue
    topo = traj.topology
    znhiscyspairs = topo.select_pairs('name ZN', \
    '(resn CYS and symbol S) or (resn HIS and symbol N)')
...
# iterate over Zn's
    ...
    # print pdb info
    print("%d %s: C4H0 %d, C3H1 %d, C2H2 %d, C1H3 %d, C0H4 %d, others %d" % \
    (progress, i, pdbC4H0count, pdbC3H1count, pdbC2H2count, pdbC1H3count, \
    pdbC0H4count, pdbotherscount))
    progress += 1

# print database info
print("ALL: C4H0 %d, C3H1 %d, C2H2 %d, C1H3 %d, C0H4 %d, others %d" % \
(globC4H0count, globC3H1count, globC2H2count, globC1H3count, globC0H4count, globotherscount))
